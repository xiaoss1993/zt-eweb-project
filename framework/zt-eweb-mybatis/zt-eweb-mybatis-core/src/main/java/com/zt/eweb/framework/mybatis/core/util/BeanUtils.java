...
import com.zt.eweb.framework.mybatis.core.converter.EnhancedConversionService;
import com.zt.eweb.framework.mybatis.core.data.copy.AcceptAnnoCopier;
...
public class BeanUtils {
   ...
   * @return
   */
  public static <T> List<T> convertMap2BeanList(List<Map<String, Object>> resultListMap,
      Class<T> entityClass) {
  ...
  public static void bindProperties(Object model, Map<String, Object> propMap) {
    ...
    for (Map.Entry<String, Object> entry : propMap.entrySet()) {
      try {
        ...
        log.debug("复制属性{}.{}异常: {}", model.getClass().getSimpleName(), entry.getKey(),
            e.getMessage());
      }
    }
  }
  ...
  public synchronized static BeanWrapper getBeanWrapper(Object obj) {
    ...
    if (conversionService == null) {
      ...
      if (conversionService == null) {
        ...
        log.debug("get ConversionService instance: {} by getBean",
            conversionService.getClass().getName());
      }
    }
  ...
  public static Object convertValueToFieldType(Object value, Class<?> fieldType) {
    ...
    if (fieldConverterMap.containsKey(fieldType)) {
      ...
      return LocalDateTime.class.equals(fieldType) ? zonedDateTime.toLocalDateTime()
   ...
   * @return
   */
  public static <T> Map<String, T> convertToStringKeyObjectMap(List<T> allLists,
      IGetter<T>... getterFns) {
    ...
    try {
      for (T model : allLists) {
        ...
        if (key != null) {
          ...
          log.warn(model.getClass().getName() + " 的属性 " + fields[0]
              + " 值存在 null，转换结果需要确认!");
        }
      }
    } catch (Exception e) {
   ...
   * @return
   */
  public static <T> Map<String, List<T>> convertToStringKeyObjectListMap(List<T> allLists,
      IGetter<T>... getterFns) {
    String[] fields = convertGettersToFields(getterFns);
  ...
  public static <T> List<T> buildTree(List<T> allNodes, Object rootNodeId, String idFieldName) {
    return buildTree(allNodes, rootNodeId, idFieldName, Cons.FieldName.parentId.name(),
        Cons.FieldName.children.name());
  }
  ...
  public static <T> List<T> buildTree(List<T> allNodes, Object rootNodeId, String idFieldName,
    ...
    for (T node : allNodes) {
      ...
      if (V.equals(nodeId, parentId)) {
        throw new BusinessException(Status.WARN_PERFORMANCE_ISSUE,
    ...
    if (parentId2ListMap.containsKey(rootNodeId)) {
      return buildTree(parentId2ListMap.remove(rootNodeId), parentId2ListMap, idFieldName,
          childrenFieldName);
  ...
  public static <T> List<T> buildTree(List<T> nodes, Map<Object, List<T>> parentId2ListMap,
      String idFieldName, String childrenFieldName) {
    for (T item : nodes) {
      Object id = getProperty(item, idFieldName);
      if (parentId2ListMap.containsKey(id)) {
        setProperty(item, childrenFieldName,
            buildTree(parentId2ListMap.remove(id), parentId2ListMap, idFieldName,
                childrenFieldName));
      }
    }
    return nodes;
   ...
   * @return Tree结构数据
   */
  public static <T> List<T> buildTree(List<T> allNodes, Serializable rootNodeId,
    ...
    for (T item : nodes) {
      ...
      if (parentId2ListMap.containsKey(id)) {
        setChildren.accept(item,
            buildTree(parentId2ListMap.remove(id), parentId2ListMap, getId, setChildren));
      }
    }
    return nodes;
  }
  ...
  /**
   * 提取两个model的差异值，只对比指定字段 使用默认的忽略字段
   *
   * @param oldModel
   * @param newModel
   * @param fields   对比字段
   ...
   * @param <T1>
   */
  public static <T1, T2, R, E> void bindPropValueOfList(ISetter<T1, R> setFieldFn, List<E> fromList,
      IGetter<T2> getFieldFun, Map valueMatchMap) {
    if (V.isEmpty(fromList)) {
      return;
    }
    // function转换为字段名
    String setterFieldName = convertToFieldName(setFieldFn), getterFieldName = convertToFieldName(
        getFieldFun);
   ...
   * @param <E>
   */
  public static <E> void bindPropValueOfList(String setterFieldName, List<E> fromList,
      String getterFieldName, Map valueMatchMap) {
  ...
  public static Class<?> getTargetClass(Object instance) {
    return (instance instanceof Class) ? (Class<?>) instance
        : AopProxyUtils.ultimateTargetClass(instance);
  }
  ...
  public static Class getGenericityClass(Object instance, int index) {
    ...
    if (resolvableType.getSuperType().getType().getTypeName().equals(Object.class.getName())
        && V.notEmpty(resolvableType.getInterfaces())) {
  ...
  public static Map<String, Object> convertLabelValueList2Map(List<LabelValue> labelValueList) {
    if (V.notEmpty(labelValueList)) {
      return labelValueList.stream()
          .collect(Collectors.toMap(LabelValue::getLabel, LabelValue::getValue));
    }
    return Collections.EMPTY_MAP;
  }
   ...
   * @return
   */
  private static List<Field> extractClassFields(Class<?> beanClazz, boolean excludeStaticFinal,
   ...
   * @param fieldNameSet
   */
  private static void loopFindFields(Class<?> beanClazz, boolean excludeSpecial,
      Class<? extends Annotation> annotation, List<Field> fieldList, Set<String> fieldNameSet) {
    if (beanClazz == null) {
      return;
    }
  ...
  private static class ModelKeyGenerator {
    ...
    /**
     * 从model中提取指定字段生成key，如果为指定字段则默认为id字段 不建议大量数据循环时调用
     *
     * @param model 提取对象
