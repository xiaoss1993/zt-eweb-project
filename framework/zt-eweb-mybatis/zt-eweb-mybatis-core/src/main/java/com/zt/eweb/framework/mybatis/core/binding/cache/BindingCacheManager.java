...
package com.zt.eweb.framework.mybatis.core.binding.cache;
...
import com.zt.eweb.framework.mybatis.core.cache.StaticMemoryCacheManager;
...
public class BindingCacheManager {
  ...
  private static final BeanInitializer<Void> ENTITY_INFO_CACHE_INITIALIZER = BeanInitUtils.lazyInit(
      () -> {
        ...
        Map<String, IService> serviceMap = ContextHolder.getApplicationContext()
            ...
            if (entityClass != null) {
              ...
              if (uniqueEntitySet.contains(entityClass.getName())) {
                if (entityIService.getClass().getAnnotation(Primary.class) != null) {
                  EntityInfoCache entityInfoCache = cacheManager.getCacheObj(
                      CACHE_NAME_CLASS_ENTITY, entityClass.getName(), EntityInfoCache.class);
                  if (entityInfoCache != null) {
                    entityInfoCache.setService(entry.getKey());
                  }
                } else {
                  log.warn("Entity: {} 存在多个service实现类，可能导致调用实例与预期不一致!",
                      entityClass.getName());
                }
              } else {
                EntityInfoCache entityInfoCache = new EntityInfoCache(entityClass, entry.getKey());
                cacheManager.putCacheObj(CACHE_NAME_CLASS_ENTITY, entityClass.getName(),
                    entityInfoCache);
                cacheManager.putCacheObj(CACHE_NAME_TABLE_ENTITY, entityInfoCache.getTableName(),
                    entityInfoCache);
                cacheManager.putCacheObj(CACHE_NAME_ENTITYNAME_CLASS, entityClass.getSimpleName(),
                    entityClass);
        ...
        if (sqlSessionFactory != null) {
          Collection<Class<?>> mappers = sqlSessionFactory.getConfiguration().getMapperRegistry()
              .getMappers();
              ...
              try {
                if (types.length > 0 && types[0] != null && types[0] instanceof ParameterizedType) {
                  ...
                  if (superTypes != null && superTypes.length > 0 && superTypes[0] != null) {
                    ...
                    if (!uniqueEntitySet.contains(entityClassName)
                        && entityClassName.length() > 1) {
                      ...
                      cacheManager.putCacheObj(CACHE_NAME_CLASS_ENTITY, entityClass.getName(),
                          entityInfoCache);
                      cacheManager.putCacheObj(CACHE_NAME_TABLE_ENTITY,
                          entityInfoCache.getTableName(), entityInfoCache);
                      cacheManager.putCacheObj(CACHE_NAME_ENTITYNAME_CLASS,
  ...
  public static EntityInfoCache getEntityInfoByClass(Class<?> entityClazz) {
    initEntityInfoCache();
    return getCacheManager().getCacheObj(CACHE_NAME_CLASS_ENTITY, entityClazz.getName(),
        EntityInfoCache.class);
  }
  ...
  public static PropInfo getPropInfoByClass(Class<?> beanClazz) {
    return getCacheManager().getCacheObj(CACHE_NAME_CLASS_PROP, beanClazz.getName(),
        () -> new PropInfo(beanClazz));
  }
  ...
  public static List<Field> getFields(Class<?> beanClazz) {
    return getCacheManager().getCacheObj(CACHE_NAME_CLASS_FIELDS, beanClazz.getName(),
        () -> BeanUtils.extractAllFields(beanClazz, true));
  }
  ...
  public static List<Field> getFields(Class<?> beanClazz, Class<? extends Annotation> annotation) {
    ...
    return getCacheManager().getCacheObj(CACHE_NAME_CLASS_FIELDS, key,
        () -> BeanUtils.extractFields(beanClazz, annotation));
  }
