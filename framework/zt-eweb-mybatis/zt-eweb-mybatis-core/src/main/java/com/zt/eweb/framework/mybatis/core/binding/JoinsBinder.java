...
public class JoinsBinder {
   ...
   * @throws Exception
   */
  private static <DTO, T> List<T> executeJoinQuery(QueryWrapper<DTO> queryWrapper,
      Class<T> entityClazz, Pagination pagination, boolean limit1) {
    // 非动态查询，走BaseService
    if (!(queryWrapper instanceof DynamicJoinQueryWrapper)) {
      IService iService = ContextHolder.getIServiceByEntity(entityClazz);
      if (iService != null) {
        return ServiceAdaptor.queryList(iService, (QueryWrapper) queryWrapper, pagination,
            entityClazz);
      } else {
        throw new InvalidUsageException(
            "单表查询对象无BaseService/IService实现: " + entityClazz.getSimpleName());
      }
    }
    ...
    if (pagination == null) {
      if (limit1) {
        ...
        IPage<Map<String, Object>> pageResult = getDynamicQueryMapper().queryForListWithPage(page,
            dynamicJoinWrapper);
      ...
      IPage<Map<String, Object>> pageResult = getDynamicQueryMapper().queryForListWithPage(
          pagination.toPage(entityClazz), dynamicJoinWrapper);
    ...
    if (ms > 5000) {
      log.warn("{} 动态Join查询执行耗时 {} ms，建议优化",
          dynamicJoinWrapper.getDtoClass().getSimpleName(), ms);
    }
    if (V.isEmpty(mapList)) {
      return Collections.emptyList();
    }
    if (mapList.size() > BaseConfig.getBatchSize()) {
      log.warn("{} 动态Join查询记录数过大( {} 条), 建议优化",
          dynamicJoinWrapper.getDtoClass().getSimpleName(), mapList.size());
    }
    // 转换查询结果
   ...
   * @param pagination
   */
  private static <T> void formatOrderBy(DynamicJoinQueryWrapper queryWrapper, Class<T> entityClazz,
      Pagination pagination) {
    // 格式化排序
    if (V.notEmpty(pagination.getOrderBy())) {
      ...
      for (String field : orderByFields) {
        ...
        List<AnnoJoiner> joinerList = ParserCache.getAnnoJoiners(queryWrapper.getDtoClass(),
