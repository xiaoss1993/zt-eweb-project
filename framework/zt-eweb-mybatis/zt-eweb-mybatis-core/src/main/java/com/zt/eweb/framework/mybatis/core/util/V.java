...
import com.zt.eweb.framework.mybatis.core.data.query.CriteriaItem;
import com.zt.eweb.framework.mybatis.core.data.query.QueryCondition;
...
public class V {
  ...
  public static boolean containsIgnoreCase(Collection<String> collection, String target) {
    return collection != null && (collection.contains(target) || collection.contains(
        target.toLowerCase()) || collection.contains(target.toUpperCase()));
  }
  ...
  public static <T> boolean equals(T source, T target) {
    if (source == target) {
      ...
      return false;
    } else if (source.getClass() != target.getClass() && !(source instanceof Map
      ...
      log.warn("暂未实现类型 " + source.getClass().getName() + "-" + target.getClass().getName()
          + " 的比对！");
      return false;
    }
  }
  ...
  public static <T> Set<ConstraintViolation<T>> validateBean(T obj, Class<?>... groups) {
    if (VALIDATOR == null) {
      VALIDATOR = Validation.byProvider(HibernateValidator.class).configure().failFast(false)
  ...
  private static class CardinalityHelper<O> {
     ...
     * </p>
     *
     * @param <O>  the type of object in the returned {@link Map}. This is a super type of
     *             &lt;I&gt;.
