...
public class SensitiveInfoSerialize<E> extends JsonSerializer<E> implements ContextualSerializer {
  ...
  public void serialize(E value, JsonGenerator gen, SerializerProvider serializers)
    ...
    if (value instanceof List) {
      gen.writeObject(((List<String>) value).stream().map(e -> dataMaskHandler.mask(e))
  ...
  public JsonSerializer<?> createContextual(SerializerProvider prov, BeanProperty property)
    ...
    if (rawClass == String.class || (rawClass == List.class
        && property.getType().getContentType().getRawClass() == String.class)) {
      DataMask protect = property.getAnnotation(DataMask.class);
      if (null == protect) {
        protect = property.getContextAnnotation(DataMask.class);
      }
      if (null != protect) {
        return new SensitiveInfoSerialize(property.getMember().getDeclaringClass(),
            property.getName());
      }
    } else {
