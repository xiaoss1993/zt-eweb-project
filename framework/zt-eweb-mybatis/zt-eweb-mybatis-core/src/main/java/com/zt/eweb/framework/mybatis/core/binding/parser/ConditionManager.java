...
public class ConditionManager extends BaseConditionManager {
   ...
   * @param <T>
   */
  private static <T> void parseDirectRelation(BaseBinder<T> binder,
    ...
    for (Expression operator : expressionList) {
      if (operator instanceof EqualsTo) {
        ...
        if (express.getRightExpression() instanceof Column) {
          ...
          // xx=this.yy，翻转
          if (isCurrentObjColumn(express.getRightExpression().toString()) && !isCurrentObjColumn(
              express.getLeftExpression().toString())) {
          ...
          if (isCurrentObjColumn(express.getLeftExpression().toString())) {
            ...
            binder.additionalCondition(
                annoColumn + " = " + express.getRightExpression().toString());
          }
        }
        continue;
        ...
        String annoColumn = removeLeftAlias(express.getLeftExpression().toString());
        if (!express.isNot()) {
          binder.andBetween(annoColumn, express.getBetweenExpressionStart().toString(),
              express.getBetweenExpressionEnd().toString());
        } else {
          binder.andNotBetween(annoColumn, express.getBetweenExpressionStart().toString(),
              express.getBetweenExpressionEnd().toString());
        }
   ...
   * @return
   */
  private static <T> List<Expression> parseMiddleTable(BaseBinder<T> binder,
      List<Expression> expressionList, String tableName) {
    // 单一条件不是中间表条件
    ...
    for (Expression operator : expressionList) {
      if (operator instanceof EqualsTo) {
        ...
        if (express.getLeftExpression() instanceof Column
            && express.getRightExpression() instanceof Column) {
