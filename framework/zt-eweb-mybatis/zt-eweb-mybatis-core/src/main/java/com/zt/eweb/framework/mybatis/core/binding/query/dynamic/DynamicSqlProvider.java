    ...
    return new SQL() {{
        ...
        for (AnnoJoiner joiner : annoJoinerList) {
          if (V.notEmpty(joiner.getJoin()) && V.notEmpty(joiner.getOnSegment())) {
            if (joiner.getMiddleTable() != null) {
              ...
              if (deletedCol != null && !S.containsIgnoreCase(joiner.getMiddleTable(),
                  " " + deletedCol)) {
                sb.append(" AND ").append(joiner.getMiddleTableAlias()).append(".")
                    .append(deletedCol).append(" = ").append(BaseConfig.getActiveFlagValue());
              }
            ...
            if (deletedCol != null && !S.containsIgnoreCase(joiner.getOnSegment(),
                " " + deletedCol)) {
              sb.append(" AND ").append(joiner.getAlias()).append(".").append(deletedCol)
                  .append(" = ").append(BaseConfig.getActiveFlagValue());
            }
            String joinSegment = sb.toString();
            if (!tempSet.contains(joinSegment)) {
              LEFT_OUTER_JOIN(joinSegment);
      ...
      if (segments != null) {
        ...
        if (V.notEmpty(normalSql)) {
          ...
          if (isDeletedCol != null && !QueryBuilder.checkHasColumn(segments.getNormal(),
              isDeletedSection)) {
          ...
          if (page != null && V.notEmpty(page.orders())) {
            ...
            page.orders().forEach(orderItem -> {
              orderByList.add(
                  S.format("%s %s", orderItem.getColumn(), orderItem.isAsc() ? "ASC" : "DESC"));
            });
    ...
    if (page != null && page.orders() != null) {
      for (OrderItem orderItem : page.orders()) {
        if ((V.isEmpty(sqlSelect) && !S.startsWith(orderItem.getColumn(), "self."))
          ...
          sb.append(Cons.SEPARATOR_COMMA).append(orderItem.getColumn()).append(" AS ")
              .append(S.replace(orderItem.getColumn(), ".", "_")).append(PLACEHOLDER_COLUMN_FLAG);
        }
      }
    }
    return sb.toString();
