...
import com.zt.eweb.framework.mybatis.core.binding.binder.remote.RemoteBindingManager;
...
public class FieldListBinder<T> extends FieldBinder<T> {
  ...
  public void bind() {
    ...
    if (middleTable == null) {
      ...
      if (V.isEmpty(this.module)) {
        ...
        // 远程调用获取
        entityList = RemoteBindingManager.fetchEntityList(module, remoteBindDTO,
    ...
    // 通过中间表关联
      ...
      // 处理中间表, 将结果转换成map
      Map<String, List> middleTableResultMap = middleTable.executeOneToManyQuery(
          trunkObjCol2ValuesMap);
      ...
      Map<String, List<T>> entityMap = BeanUtils.convertToStringKeyObjectListMap(entityList,
          refObjJoinOnField);
      ...
      // 遍历list并赋值
      bindPropValue(super.getMatchedAnnoObjectList(), middleTable.getTrunkObjColMapping(),
          valueEntityListMap);
    }
  }
   ...
   * @param <E>
   */
  private <E> void bindPropValue(List<E> fromList, Map<String, String> trunkObjColMapping,
      ...
      for (E object : fromList) {
        ...
        if (entityList != null) {
          ...
          for (int i = 0; i < annoObjectSetterPropNameList.size(); i++) {
            List valObjList = BeanUtils.collectToList(entityList,
                referencedGetterFieldNameList.get(i));
