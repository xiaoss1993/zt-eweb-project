...
import com.zt.eweb.framework.mybatis.core.binding.parser.BaseConditionManager;
    ...
    for (Expression operator : expressionList) {
      ...
      if (operator instanceof BinaryExpression) {
        ...
        if (joiner.getMiddleTable() != null &&
            (left.startsWith(joiner.getMiddleTableAlias() + ".") || right.startsWith(
                joiner.getMiddleTableAlias() + "."))) {
          if (!left.startsWith(joiner.getAlias() + ".") && !right.startsWith(
              joiner.getAlias() + ".")) {
        ...
        String left = formatColumn(expression.getLeftExpression(), joiner);
        // 中间表条件
        if (joiner.getMiddleTable() != null && left.startsWith(
            joiner.getMiddleTableAlias() + ".")) {
          currentSegments = middleTableOnSegments;
        }
        if (!expression.isNot()) {
        ...
        if (!expression.isNot()) {
          currentSegments.add(
              left + " BETWEEN " + expression.getBetweenExpressionStart().toString() + " AND "
                  + expression.getBetweenExpressionEnd().toString());
        } else {
          currentSegments.add(
              left + " NOT BETWEEN " + expression.getBetweenExpressionStart().toString() + " AND "
                  + expression.getBetweenExpressionEnd().toString());
        }
      } else {
