 ...
 * @author JerryMa
 * @version v3.0.0
 * @date 2023/2/15 Copyright © diboot.com
 */
@Slf4j
...
public class QueryCondition implements Serializable {
  ...
  public <T, FT> QueryCondition between(SFunction<T, FT> fieldGetter, Object beginValue,
      Object endValue) {
  ...
  public <T, FT> QueryCondition appendCriteria(SFunction<T, FT> fieldGetter, Comparison comparison,
      Object value) {
    return appendCriteria(BeanUtils.convertSFunctionToFieldName(fieldGetter), comparison, value);
  }

  public <T, FT> QueryCondition appendCriteria(String fieldName, Comparison comparison,
      Object value) {
   ...
   * @return
   */
  public <T, FT> QueryCondition updateCriteria(SFunction<T, FT> fieldGetter, Comparison comparison,
      Object value) {
  ...
  public <T> QueryWrapper<T> toQueryWrapper(Class<T> entityClass) {
    ...
    if (V.notEmpty(selectFields)) {
      query.select(
          selectFields.stream().map(entityInfo::getColumnByField).collect(Collectors.toList()));
    }
    // 解析条件
  ...
  public boolean isDefaultOrder() {
    if (V.notEmpty(orderItems) && orderItems.size() == 1 && orderItems.get(0)
        .equals(Pagination.ORDER_BY_ID_DESC)) {
      return true;
    }
    return false;
  }
