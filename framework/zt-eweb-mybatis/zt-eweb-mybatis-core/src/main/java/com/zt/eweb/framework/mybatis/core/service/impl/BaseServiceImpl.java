...
package com.zt.eweb.framework.mybatis.core.service.impl;
...
import com.zt.eweb.framework.mybatis.core.binding.query.dynamic.DynamicJoinQueryWrapper;
import com.zt.eweb.framework.mybatis.core.config.BaseConfig;
import com.zt.eweb.framework.mybatis.core.config.Cons;
import com.zt.eweb.framework.mybatis.core.dto.SortParamDTO;
import com.zt.eweb.framework.mybatis.core.entity.BaseTreeEntity;
...
public class BaseServiceImpl<M extends BaseCrudMapper<T>, T> extends ServiceImpl<M, T> implements
    BaseService<T> {
  ...
  @Override
  public <FT> FT getValueOfField(SFunction<T, ?> queryFieldFn, Serializable queryFieldVal,
      SFunction<T, FT> getterFn) {
  ...
  @Override
  @Transactional(rollbackFor = Exception.class)
  public <RE, R> boolean createEntityAndRelatedEntities(T entity, List<RE> relatedEntities,
  ...
  @Override
  public <RE, R> boolean createRelatedEntities(Serializable entityId, List<RE> relatedEntities,
      ISetter<RE, R> relatedEntitySetter) {
    if (V.isEmpty(relatedEntities)) {
      return true;
    }
    ...
    relatedEntities.forEach(
        relatedEntity -> BeanUtils.setProperty(relatedEntity, relatedEntitySetterFld, entityId));
    // 获取关联对象对应的Service
  ...
  @Override
  public <RE, R> boolean createRelatedEntity(Serializable entityId, RE relatedEntity,
      ISetter<RE, R> relatedEntitySetter) {
  ...
  public <R> boolean createOrUpdateN2NRelations(SFunction<R, ?> driverIdGetter, Object driverId,
    ...
    return createOrUpdateN2NRelations(driverIdGetter, driverId, followerIdGetter, followerIdList,
        null, null);
  }
  ...
  public <R> boolean createOrUpdateN2NRelations(SFunction<R, ?> driverIdGetter, Object driverId,
    ...
    if (entityInfo == null) {
      throw new InvalidUsageException(
          "未找到 " + middleTableClass.getName() + " 的 Service 或 Mapper 定义！");
    }
    ...
    List<R> oldEntityList =
        (iService != null) ? iService.list(selectOld) : baseMapper.selectList(selectOld);
    // 删除失效关联
    ...
    for (R entity : oldEntityList) {
      ...
      Serializable id = (Serializable) BeanUtils.getProperty(entity,
          isExistPk ? entityInfo.getPropInfo().getIdFieldName() : followerFieldName);
      if (id != null) {
        delIds.add(id);
      }
    }
  ...
  @Override
  @Transactional(rollbackFor = Exception.class)
  public <RE, R> boolean updateEntityAndRelatedEntities(T entity, List<RE> relatedEntities,
      ISetter<RE, R> relatedEntitySetter) {
  ...
  @Override
  public <RE, R> boolean updateRelatedEntities(Serializable entityId, List<RE> relatedEntities,
      ISetter<RE, R> relatedEntitySetter) {
    // 获取关联entity的类
    ...
    if (V.notEmpty(relatedEntities)) {
      ...
      try {
        relatedEntityClass = Class.forName(
            BeanUtils.getSerializedLambda(relatedEntitySetter).getImplClass().replaceAll("/", "."));
      } catch (Exception e) {
  ...
  @Override
  public <RE, R> boolean updateRelatedEntity(Serializable entityId, RE relatedEntity,
      ISetter<RE, R> relatedEntitySetter) {
    // 获取关联entity的类
    Class relatedEntityClass;
    if (relatedEntity != null) {
  ...
  @Override
  @Transactional(rollbackFor = Exception.class)
  public <RE, R> boolean deleteEntityAndRelatedEntities(Serializable id,
      Class<RE> relatedEntityClass, ISetter<RE, R> relatedEntitySetter) {
  ...
  @Override
  public <RE, R> boolean deleteRelatedEntities(Object entityIds, Class<RE> relatedEntityClass,
      ISetter<RE, R> relatedEntitySetter) {
    // 获取主键的关联属性
    PropInfo propInfo = BindingCacheManager.getPropInfoByClass(relatedEntityClass);
    String relatedEntitySetterFld = BeanUtils.convertToFieldName(relatedEntitySetter);
  ...
  @Override
  public <RE, R> boolean deleteRelatedEntity(Object entityIds, Class<RE> relatedEntityClass,
      ISetter<RE, R> relatedEntitySetter) {
  ...
  public boolean deleteEntities(Wrapper queryWrapper) {
    ...
    if (queryWrapper instanceof QueryWrapper) {
        String idCol = ContextHolder.getIdColumnName(getEntityClass());
  ...
  public <FT> List<FT> getValuesOfField(Wrapper queryWrapper, SFunction<T, FT> getterFn) {
    ...
    if (queryWrapper instanceof QueryWrapper) {
      ...
      throw new InvalidUsageException(
          "不支持的Wrapper类型：" + (queryWrapper == null ? "null" : queryWrapper.getClass()));
    }
    // 如果是动态join，则调用JoinsBinder
    ...
    return entityList.stream().filter(Objects::nonNull).map(getterFn).distinct()
        .collect(Collectors.toList());
  }

  @Override
  public <ST, FT> Map<ST, FT> getValueMapOfField(SFunction<T, ST> idFieldFn, List<ST> idValList,
      SFunction<T, FT> getterFn) {
  ...
  public List<LabelValue> getLabelValueList(Wrapper queryWrapper) {
    ...
    if (V.isEmpty(sqlSelect) || S.countMatches(sqlSelect, Cons.SEPARATOR_COMMA) < 1) {
      throw new InvalidUsageException(
          "调用错误: getLabelValueList必须用select依次指定返回的 label,value(,ext)键值字段，如: new QueryWrapper<Dictionary>().lambda().select(Dictionary::getItemName, Dictionary::getItemValue)");
    }
    List<T> entityList = getEntityList(queryWrapper);
    if (entityList == null) {
      return Collections.emptyList();
    ...
    for (T entity : entityList) {
      if (V.isEmpty(entity)) {
        log.warn("getLabelValueList查询指定字段 {} 在数据库当前记录所有字段均为空值，已自动忽略",
            sqlSelect);
        continue;
      }
      String label = propInfo.getFieldByColumn(selectArray[0]), value = propInfo.getFieldByColumn(
          selectArray[1]), ext;
    ...
    if (hasParentNode) {
      return BeanUtils.buildTree(labelValueList, LabelValue::getValue, LabelValue::getParentId,
          LabelValue::setChildren);
    }
  ...
  public <ID> Map<ID, String> getId2NameMap(List<ID> entityIds, SFunction<T, ?> getterFn) {
    if (V.isEmpty(entityIds)) {
      ...
      String columnName = entityInfo.getColumnByField(
              BeanUtils.convertSFunctionToFieldName(getterFn));
  ...
  @Override
  public <VO> List<VO> getViewObjectList(Wrapper queryWrapper, Pagination pagination,
      Class<VO> voClass) {
  ...
  @Override
  public boolean sort(SortParamDTO<?> sortParam, SFunction<T, Number> sortField,
      SFunction<T, Serializable> parentIdField, SFunction<T, String> parentIdsField) {
    ...
    if (levelChange) {
      ...
      String parentIdFieldName = PropertyNamer.methodToProperty(
          parentIdFieldLambda.getImplMethodName());
      ...
      if (parentIdsField != null) {
        ...
        String parentIdsFieldName = PropertyNamer.methodToProperty(
            parentIdsFieldLambda.getImplMethodName());
        BiFunction<String, Serializable, String> buildPids = (pids, idValue) -> V.isEmpty(pids)
            ? S.valueOf(idValue) : pids + "," + idValue;
        String parentIds = buildPids.apply(
            parentEntity == null ? null : parentIdsField.apply(parentEntity), newParentId);
        ...
        QueryWrapper<T> queryWrapper = Wrappers.<T>query().select(idColumn, parentPathColumnName)
            .likeRight(parentPathColumnName,
                buildPids.apply(parentIdsField.apply(getEntity(id)), id));
        List<T> entityList = getEntityList(queryWrapper);
        if (V.notEmpty(entityList)) {
          Map<Serializable, List<T>> pid2entityMap = entityList.stream()
              .collect(Collectors.groupingBy(parentIdField));
          updateParentIds(idFieldName, pid2entityMap, id, buildPids.apply(parentIds, id),
              parentIdsFieldName);
   ...
   */
  private void updateParentIds(String idFieldName, Map<Serializable, List<T>> pid2entityMap,
      Serializable pid, String parentIds, String parentIdsFieldName) {
    for (T item : pid2entityMap.get(pid)) {
      ...
      if (pid2entityMap.containsKey(idValue)) {
        updateParentIds(idFieldName, pid2entityMap, idValue, parentIds + "," + idValue,
            parentIdsFieldName);
      }
    }
  }
