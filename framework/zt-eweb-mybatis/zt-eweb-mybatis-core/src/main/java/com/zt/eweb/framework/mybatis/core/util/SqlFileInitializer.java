...
import com.zt.eweb.framework.mybatis.core.util.sql.*;
...
public class SqlFileInitializer {
  ...
  public static List<String> extractSqlStatements(Class inst, String sqlPath) {
    ...
    if (DbType.MYSQL.getDb().equalsIgnoreCase(dbType) || DbType.MARIADB.getDb()
        .equalsIgnoreCase(dbType)) {
      return sqlStatements;
    }
    // PostgresSql
    else if (DbType.POSTGRE_SQL.getDb().equalsIgnoreCase(dbType) || DbType.KINGBASE_ES.getDb()
        .equalsIgnoreCase(dbType)) {
  ...
  private static String removeMultipleLineComments(String inputSql) {
    ...
    if (inputSql.contains("/*") && inputSql.contains("*/")) {
      inputSql = inputSql.substring(0, inputSql.indexOf("/*")) + inputSql.substring(
          inputSql.indexOf("*/") + 2);
    }
    if (inputSql.contains("/*") && inputSql.contains("*/")) {
