...
public class BindAnnotationGroup {
  ...
  public void addBindAnnotation(String fieldName, Class<?> fieldClass, Annotation annotation) {
    ...
    try {
      if (annotation instanceof BindField) {
        ...
        key =
            bindField.entity().getName() + ":" + bindField.condition() + ":" + bindField.orderBy();
    ...
    if (annotation instanceof BindField) {
      ...
      List<FieldAnnotation> list = bindFieldListGroupMap.computeIfAbsent(key,
          k -> new ArrayList<>(4));
      list.add(fieldAnnotation);
    } else if (annotation instanceof BindCount) {
  ...
  public boolean isEmpty() {
    return V.isAllEmpty(bindDictAnnotations, bindFieldGroupMap, bindEntityAnnotations,
        bindEntityListAnnotations, bindFieldListGroupMap, bindCountAnnotations,
        bindI18nAnnotations);
  }
