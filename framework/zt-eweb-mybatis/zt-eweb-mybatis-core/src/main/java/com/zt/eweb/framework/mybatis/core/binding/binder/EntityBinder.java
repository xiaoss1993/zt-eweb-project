    ...
    if (middleTable == null) {
      ...
      if (V.notEmpty(entityList)) {
        ...
        ResultAssembler.bindPropValue(annoObjectField, super.getMatchedAnnoObjectList(),
            getAnnoObjJoinFlds(), valueEntityMap, null);
      }
    }
    // 通过中间表关联Entity
    else {
      ...
      Map<String, Object> middleTableResultMap = middleTable.executeOneToOneQuery(
      ...
      this.simplifySelectColumns();
      // 提取entity主键值集合
      Collection refObjValues = middleTableResultMap.values().stream().distinct()
