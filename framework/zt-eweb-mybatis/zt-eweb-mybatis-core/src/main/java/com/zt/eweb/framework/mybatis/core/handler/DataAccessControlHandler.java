 ...
 * the License.
 */
package com.zt.eweb.framework.mybatis.core.handler;

import com.baomidou.mybatisplus.core.metadata.TableInfo;
import com.baomidou.mybatisplus.core.metadata.TableInfoHelper;
import com.baomidou.mybatisplus.extension.plugins.handler.MultiDataPermissionHandler;
import com.zt.eweb.framework.mybatis.core.data.access.DataAccessAnnoCache;
import com.zt.eweb.framework.mybatis.core.data.access.DataScopeManager;
...
public class DataAccessControlHandler implements MultiDataPermissionHandler {
  ...
  public Expression getSqlSegment(Table table, Expression where, String mappedStatementId) {
    ...
    if (tableInfo == null || tableInfo.getEntityType() == null
        || !DataAccessAnnoCache.hasDataAccessCheckpoint(tableInfo.getEntityType())) {
  ...
  private Expression buildDataAccessExpression(Table mainTable, Class<?> entityClass) {
    return DataAccessAnnoCache.getDataPermissionMap(entityClass).entrySet().stream().map(entry -> {
      ...
      List<? extends Serializable> idValues = checkImpl.getAccessibleIds(entityClass,
          entry.getKey());
      if (idValues == null) {
        return null;
      }
