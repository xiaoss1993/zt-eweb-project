...
package com.zt.eweb.framework.mybatis.core.controller;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.toolkit.Wrappers;
import com.zt.eweb.framework.mybatis.core.binding.Binder;
import com.zt.eweb.framework.mybatis.core.binding.QueryBuilder;
...
public class BaseController {
   ...
   * @return
   */
  protected <DTO> QueryWrapper<DTO> buildQueryWrapperByDTO(DTO entityOrDto, Pagination pagination)
      throws Exception {
   ...
   */
  @Deprecated
  protected <DTO> QueryWrapper<DTO> buildQueryWrapperByQueryParams(DTO entityOrDto)
      throws Exception {
   ...
   */
  protected List<LabelValue> loadRelatedData(RelatedDataDTO relatedDataDTO,
      @Nullable String parentId, @Nullable String keyword) {
      ...
      if (V.notEmpty(field)) {
        ...
        if (V.notEmpty(column)) {
          ...
          throw new BusinessException(
              "relatedData: " + relatedDataDTO.getType() + " 无 `" + field + "` 属性");
        }
      }
      return null;
    };
    ...
    if (V.notEmpty(relatedDataDTO.getParent())) {
      ...
      Function<String, Serializable> parentIdTypeConversion = pid -> V.isEmpty(pid) ? null
          : parentFieldType == Long.class ? Long.valueOf(pid) : pid;
      ...
      if (V.notEmpty(relatedDataDTO.getConditions())) {
        Optional<BaseCriteria> parentCriteriaOpt = relatedDataDTO.getConditions().stream()
            .filter(criteria -> criteria.getField().equals(relatedDataDTO.getParent())).findFirst();
      ...
      boolean isDynamicRoot =
          !parentIdTypeConversion.apply(Cons.TREE_ROOT_ID).equals(rootId) && V.isNoneEmpty(rootId,
              parentPathColumn);
      if (isDynamicRoot) {
        ...
        String parentPath = S.valueOf(
            BeanUtils.getProperty(rootNode, relatedDataDTO.getParentPath()));
        queryWrapper.and(query -> {
          query.likeRight(parentPathColumn, V.isEmpty(parentPath) ? S.valueOf(rootId)
              : parentPath + Cons.SEPARATOR_COMMA + rootId);
      ...
      if (V.notEmpty(parentId)) {
        ...
        List<Map<String, Object>> mapList = baseService.getMapList(
            Wrappers.query().select(idColumn, parentPathColumn).like(label, keyword));
        if (V.isEmpty(mapList)) {
          return Collections.emptyList();
        }
        queryWrapper.in(idColumn, new HashSet<Serializable>() {{
          addAll(mapList.stream()
              .peek(map -> add(parentIdTypeConversion.apply(S.valueOf(map.remove(idColumn)))))
    ...
    // 获取数据并做相应填充
    List<LabelValue> labelValueList = baseService.getLabelValueList(
        queryWrapper.select(columns.toArray(new String[]{})));
    if (V.isEmpty(keyword) && labelValueList.size() > BaseConfig.getBatchSize()) {
      log.warn("relatedData: {} 数据量超过 {} 条, 建议采用远程搜索接口过滤数据", entityClassName,
   ...
   */
  protected void buildRelatedDataCondition(RelatedDataDTO relatedDataDTO,
      QueryWrapper<?> queryWrapper, Function<String, String> field2column) {
    ...
    relatedDataDTO.getConditions().forEach(criteria -> {
      ...
      // 根据匹配方式构建查询
      WrapperHelper.buildQueryCriteria(queryWrapper, criteria.getComparison(), columnName,
          criteria.getValue());
    });
  }
