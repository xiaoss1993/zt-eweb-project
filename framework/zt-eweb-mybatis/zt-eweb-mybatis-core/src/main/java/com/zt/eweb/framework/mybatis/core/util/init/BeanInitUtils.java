...
public class BeanInitUtils {
  ...
  public static <T> BeanInitializer<T> lazyInit(Class<T> beanClass) {
    return new LazyBeanInitializer<T>() {
      @Override
      protected T initialize() {
        return Objects.requireNonNull(ContextHolder.getBean(beanClass),
            () -> "找不到class对应的bean: " + beanClass.getName());
      }
    };
  }
  ...
  private static abstract class LazyBeanInitializer<T> implements BeanInitializer<T> {
    ...
    /**
     * Returns the object wrapped by this instance. On first access the object is created. After
     * that it is cached and can be accessed pretty fast.
     *
     ...
     * called by {@link #get()} when the object is accessed for the first time. An implementation
     * can focus on the creation of the object. No synchronization is needed, as this is already
     * handled by {@code get()}.
     *
