...
package com.zt.eweb.framework.mybatis.core.binding;
...
import com.zt.eweb.framework.mybatis.core.binding.query.dynamic.AnnoJoiner;
import com.zt.eweb.framework.mybatis.core.binding.query.dynamic.DynamicJoinQueryWrapper;
import com.zt.eweb.framework.mybatis.core.binding.query.dynamic.ExtQueryWrapper;
...
public class QueryBuilder {
  ...
  private static <DTO> QueryWrapper<?> dtoToWrapper(DTO dto, Collection<String> fields,
    ...
    BiPredicate<Object, BindQuery> ignoreEmpty = (value, bindQuery) -> bindQuery != null &&
        (Strategy.IGNORE_EMPTY.equals(bindQuery.strategy()) && value instanceof String && S.isEmpty(
            (String) value) // 忽略空字符串""
            || Comparison.IN.equals(bindQuery.comparison()) && V.isEmpty(value)); // 忽略空集合
    // 获取Class类型
    Function<BindQuery, Class<?>> getClass = bindQuery ->
        bindQuery == null || bindQuery.entity() == NullType.class ? dto.getClass()
            : bindQuery.entity();
    // 获取属性名类型
    BiFunction<BindQuery, String, String> getFieldName = (bindQuery, defFieldName) ->
        bindQuery == null || S.isEmpty(bindQuery.column()) ? defFieldName : bindQuery.column();
    // 保护字段处理器
    ...
    for (Map.Entry<String, FieldAndValue> entry : fieldValuesMap.entrySet()) {
      ...
      // 构建Query
      if (queryList != null) {
        List<BindQuery> bindQueryList = Arrays.stream(queryList.value())
            ...
            if (protectFieldHandler != null) {
              ...
              if (encryptFields.contains(fieldName)) {
                ...
                buildQuery(queryWrapper.or(), bindQuery, columnName,
                    protectFieldHandler.encrypt(value.toString()));
                continue;
              }
            }
   ...
   * @param value      值
   */
  private static void buildQuery(QueryWrapper<?> wrapper, BindQuery bindQuery, String columnName,
      Object value) {
   ...
   * @return
   */
  private static <DTO> LinkedHashMap<String, FieldAndValue> extractNotNullValues(DTO dto,
      Collection<String> fields, Pagination pagination) {
    ...
    for (Field field : declaredFields) {
      ...
      if (V.notContains(fields, fieldName)) {
        //Date 属性放过
        if (!V.equals(field.getType(), Date.class) && !V.equals(field.getType(), LocalDate.class)
            && !V.equals(field.getType(), LocalDateTime.class)) {
          continue;
        }
      }
      ...
      if (field.getType().getName().equals("boolean")) {
        ...
        if (tableLogic == null) {
          ...
          if (tableField == null || tableField.exist()) {
            log.warn(
                "{}.{} 字段类型为 boolean，其默认值将参与构建查询条件，可能导致结果与预期不符，建议调整为 Boolean 类型 或 指定 @BindQuery(ignore=true)",
