...
public class WrapperHelper {
   ...
   * @param value      值
   */
  public static void buildQueryCriteria(QueryWrapper<?> wrapper, Comparison comparison,
   ...
   */
  public static void buildOrderBy(QueryWrapper<?> query, String orderBy,
      Function<String, String> field2column) {
    // 解析排序
  ...
  public static Predicate<TableFieldInfo> buildSelectPredicate(Class<?> voClass) {
    List<String> fieldList = BindingCacheManager.getFields(voClass).stream().flatMap(field -> {
      ...
      return accept == null ? Stream.of(field.getName())
          : accept.override() ? Stream.of(accept.name())
              : Stream.of(field.getName(), accept.name());
    }).collect(Collectors.toList());
    return fieldInfo -> fieldList.contains(fieldInfo.getField().getName())
