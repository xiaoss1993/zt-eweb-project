 ...
 * @author JerryMa
 * @version v2.2.1
 * @date 2021/4/17 Copyright © diboot.com
 */
@Slf4j
public abstract class BaseMemoryCacheManager extends SimpleCacheManager implements
    BaseCacheManager {
  ...
  public void putCacheObj(String cacheName, Object objKey, Object obj) {
    ...
    if (cache != null) {
      ...
      if (log.isDebugEnabled()) {
        ...
        log.debug("缓存: {} 新增-> {} , 当前size={}", cacheName, objKey,
            mapCache.getNativeCache().size());
      }
    } else {
  ...
  public void removeCacheObj(String cacheName, Object objKey) {
    ...
    if (cache != null) {
      ...
      if (log.isDebugEnabled()) {
        ConcurrentMapCache mapCache = (ConcurrentMapCache) cache;
        log.debug("缓存删除: {}.{} , 当前size={}", cacheName, objKey,
            mapCache.getNativeCache().size());
      }
    } else {
      throw new InvalidUsageException("无法获取cache：{}，请检查是否初始化", cacheName);
