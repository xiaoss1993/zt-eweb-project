...
public class FieldBinder<T> extends BaseBinder<T> {
  ...
  public <T1, T2, R> FieldBinder<T> link(IGetter<T2> fromDoGetter, ISetter<T1, R> toVoSetter) {
    return link(BeanUtils.convertToFieldName(fromDoGetter),
        BeanUtils.convertToFieldName(toVoSetter));
  }
   ...
   * @param matchKey
   */
  private void setFieldValueToTrunkObj(Map<String, T> key2EntityMap, Object annoObject,
      String matchKey) {
