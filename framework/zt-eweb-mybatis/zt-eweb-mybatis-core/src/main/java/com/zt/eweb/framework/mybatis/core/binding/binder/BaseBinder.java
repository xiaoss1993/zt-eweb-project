...
package com.zt.eweb.framework.mybatis.core.binding.binder;

import com.baomidou.mybatisplus.core.conditions.Wrapper;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.service.IService;
import com.zt.eweb.framework.mybatis.core.binding.annotation.Module;
import com.zt.eweb.framework.mybatis.core.binding.binder.remote.RemoteBindDTO;
import com.zt.eweb.framework.mybatis.core.binding.cache.BindingCacheManager;
import com.zt.eweb.framework.mybatis.core.binding.helper.ResultAssembler;
import com.zt.eweb.framework.mybatis.core.binding.helper.WrapperHelper;
import com.zt.eweb.framework.mybatis.core.binding.parser.FieldComparison;
import com.zt.eweb.framework.mybatis.core.binding.parser.MiddleTable;
...
public abstract class BaseBinder<T> {
  ...
  /**
   * 多对多关联的桥接表，如 user_role<br> 多对多注解示例: id=user_role.user_id AND user_role.role_id=id
   */
   ...
   */
  @Deprecated
  public <T1, T2> BaseBinder<T> joinOn(IGetter<T1> annoObjectFkGetter,
      IGetter<T2> referencedEntityPkGetter) {
    String annoObjectForeignKey = toAnnoObjColumn(BeanUtils.convertToFieldName(annoObjectFkGetter));
    String referencedEntityPrimaryKey = toRefObjColumn(
        BeanUtils.convertToFieldName(referencedEntityPkGetter));
   ...
   * @return
   */
  public BaseBinder<T> joinOnFieldComparison(String annoObjectFieldKey, Comparison comparison,
      Object eqFilterConsVal) {
    if (annoObjectFieldKey != null && eqFilterConsVal != null) {
      ...
      if (fieldName == null && this.annoObjPropInfo.getFieldToColumnMap()
          .containsKey(annoObjectFieldKey)) {
  ...
  public BaseBinder<T> andBetween(String fieldName, Object begin, Object end) {
    queryWrapper.between(toRefObjColumn(fieldName), formatValue(fieldName, begin),
        formatValue(fieldName, end));
    return this;
  }
  ...
  public BaseBinder<T> andNotBetween(String fieldName, Object begin, Object end) {
    queryWrapper.notBetween(toRefObjColumn(fieldName), formatValue(fieldName, begin),
        formatValue(fieldName, end));
    return this;
  }
  ...
  protected void buildQueryWrapperJoinOn() {
    for (int i = 0; i < annoObjJoinCols.size(); i++) {
      ...
      List<?> annoObjectJoinOnList = BeanUtils.collectToList(getMatchedAnnoObjectList(),
          toAnnoObjField(annoObjJoinOnCol));
      // 构建查询条件
      if (V.notEmpty(annoObjectJoinOnList)) {
        ...
        List<?> unpackAnnoObjectJoinOnList = ResultAssembler.unpackValueList(annoObjectJoinOnList,
            this.splitBy, fieldType);
        queryWrapper.in(refObjJoinOnCol, unpackAnnoObjectJoinOnList);
        if (remoteBindDTO != null) {
          remoteBindDTO.setRefJoinCol(refObjJoinOnCol)
              .setInConditionValues(unpackAnnoObjectJoinOnList);
        }
      }
    }
  }
    ...
    if (iService == null) {
      ...
      if (moduleAnno == null) {
        throw new InvalidUsageException(
            entityClass.getSimpleName() + " 无 BaseService/IService实现类，无法执行注解绑定！");
      }
    }
    return iService;
  }
