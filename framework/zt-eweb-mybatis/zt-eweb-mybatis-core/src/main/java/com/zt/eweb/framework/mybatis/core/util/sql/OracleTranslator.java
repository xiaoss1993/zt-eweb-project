...
public class OracleTranslator extends BaseTranslator {
  ...
  protected List<String> formatStatements(List<String> otherStatements) {
    return otherStatements.stream().map(s -> S.substringBefore(s, ";"))
  ...
  protected String translateColDefineSql(String colDefineSql) {
    colDefineSql = S.replaceEach(colDefineSql,
        new String[]{" tinyint(1)", " tinyint", "varchar(", " datetime ", " bigint ", " json ",
            " JSON ", " text"},
        new String[]{" NUMBER(1)", " NUMBER(1)", "VARCHAR2(", " TIMESTAMP ", " NUMBER(20) ",
            " VARCHAR2(1000) ", " VARCHAR2(1000) ", " CLOB"}
    );
    ...
    if (S.contains(colDefineSql, " DEFAULT ")) {
      ...
      prefix = S.replaceEach(prefix, new String[]{" not null ", " null "},
          new String[]{" NOT NULL ", " NULL "});
      if (S.contains(prefix, " NOT NULL ")) {
        colDefineSql =
            S.substringBefore(prefix, " NOT NULL ") + " DEFAULT " + defValue + " NOT NULL "
                + S.substringAfter(prefix, " NOT NULL ")
                + suffix;
      } else if (S.contains(prefix, " NULL ")) {
        colDefineSql = S.substringBefore(prefix, " NULL ") + " DEFAULT " + defValue + " NULL "
            + S.substringAfter(prefix, " NULL ")
            + suffix;
      }
    }
    ...
    if (insertIntoPrefix.contains("`")) {
      for (String key : ESCAPE_KEYWORDS) {
        if (insertIntoPrefix.contains("`" + key + "`")) {
          insertIntoPrefix = S.replace(insertIntoPrefix, "`" + key + "`",
              "\"" + key.toUpperCase() + "\"");
        }
      }
    }
