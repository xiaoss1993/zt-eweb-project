...
public class SqliteTranslator extends BaseTranslator {
  ...
  protected String translateColDefineSql(String colDefineSql) {
    ...
    colDefineSql = S.replaceEach(colDefineSql,
        new String[]{" smallint ", " int ", " tinyint(1) ", " tinyint ", " datetime ", " bigint ",
            " json ", " JSON ", " text", " VARCHAR("},
        new String[]{" INTEGER ", " INTEGER ", " INTEGER ", " INTEGER ", " TEXT ", " INTEGER ",
            " TEXT ", " TEXT ", " TEXT", " varchar("}
    );
    if (S.contains(colDefineSql, "varchar(")) {
      colDefineSql =
          S.substringBefore(colDefineSql, "varchar(") + " TEXT " + S.substringAfter(colDefineSql,
              ") ");
    }
