# 以下在服务器上操作
# 1. 停止容器
docker stop zt-eweb-lite-container
# 2. 删除容器
docker rm zt-eweb-lite-container
# 3. 删除镜像
docker rmi zt-eweb-lite
# 4. 加载镜像
docker load -i zt-eweb-lite.tar
...
# 直接运行
docker run -p 8888:8888 -d  --name zt-eweb-lite-container zt-eweb-lite
# 6. 查看运行情况
docker ps -a -f name=zt-eweb-lite-container
# 7. 查看指定名称的镜像
docker images zt-eweb-lite
# 8. 查看运行日志
# 实时查看最新前500行日志
docker logs -f -t --tail 500 zt-eweb-lite-container
# 查看物理主机上映射的日志
